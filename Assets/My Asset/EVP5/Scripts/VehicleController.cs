...
namespace EVP
{
    ...
    public class WheelData
    {
        ...
        public bool
            visualGrounded =
                false; // Result of raycast for visual wheels. Used for precise positioning (i.e. tire marks).

        public RaycastHit visualHit;

        public Collider
            visualHitCollider; // Sometimes the collider is taken from the ground hit, but RaycastHit.collider is read-only.
    ...
    public class VehicleController : MonoBehaviour
    {
        ...
        [Header("Optimization & Debug")] public bool disallowRuntimeChanges = false;
            ...
            if (wd.grounded)
            {
                ...
                if (wd.isBraking)
                {
                    ...
                    minSlipY = Mathf.Clamp(Mathf.Abs(demandedForce * wd.tireSlip.x) / forceMagnitude, 0.0f,
                        wheelMaxBrakeSlip+150);//thêm tốc độ tối đa cho xe
                }
                else
                {
                ...
                wd.dragForce = -(forceMagnitude * wd.localVelocity.magnitude * groundDrag * upSpeed) * wd.localVelocity;
            }
            else
            {
        ...
        public void ResetVehicle() //reset vehicle về nơi start
        {
